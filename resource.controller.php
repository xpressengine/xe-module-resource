...
class resourceController extends resource
{
	...
	function procResourceInsertPackage()
	{
		...
		foreach($args as $key => $val) if(!trim($val))
		{
			return new Object(-1,'msg_invalid_request');
		}
	...
	function insertDependency($module_srl, $package_srl, $item_srl, $targets)
	{
		...
		{
			if($val->package_srl == $package_srl || $val->item_srl == $item_srl)
			{
				continue;
			}
	...
	function procResourceAttachFile($proc = TRUE)
	{
		...
		if(!$args->package_srl || !$args->item_srl)
		{
			return new Object(-1,'msg_invalid_request');
		}

		if(!is_uploaded_file($args->attach_file['tmp_name']))
		{
			return new Object(-1,'msg_invalid_request');
		}
		if(!is_uploaded_file($args->attach_screenshot['tmp_name']))
		{
			return new Object(-1,'msg_invalid_request');
		}
		...
		if(!$this->grant->manager && $logged_info->member_srl != $package->member_srl)
		{
			return new Object(-1,'msg_not_permitted');
		}
	...
	function procResourceModifyAttach()
	{
		...
		if(!$this->module_srl || !$package_srl || !$item_srl)
		{
			return new Object(-1,'msg_invalid_request');
		}
	...
	function procResourceDeleteAttach()
	{
		...
		$package = $oResourceModel->getPackage($this->module_srl, $package_srl);
		if(!$package || (!$this->grant->manager && $package->member_srl != $logged_info->member_srl))
		{
			return new Object(-1,'msg_invalid_request');
		}
	...
	function procResourceInsertComment()
	{
		...
		if(!$args->star_point || !$args->content || !$args->package_srl || !$args->item_srl)
		{
			return new Object(-1,'msg_invalid_request');
		}

		$item = $oResourceModel->getItem($args->module_srl, $args->package_srl, $args->item_srl);
		if(!$item->document_srl)
		{
			return new Object(-1,'msg_invalid_request');
		}

		$package = $oResourceModel->getPackage($args->module_srl, $args->package_srl);
		if(!$package->package_srl)
		{
			return new Object(-1,'msg_invalid_request');
		}

		$logged_info = Context::get('logged_info');
		if($oResourceModel->hasVoted($this->module_srl, $package->package_srl, $args->item_srl, $logged_info->member_srl))
		{
			return new Object(-1,'msg_already_voted');
		}
