	...
	function dispResourceIndex()
	{
		...
		if($latest_package)
		{
			...
			if($output->data)
			{
				...
				if($package->get('package_voter') > 0)
				{
					$package->add('star', (int)($package->get('package_voted')/$package->get('package_voter')));
				}
				else
				{
					$package->add('star',0);
				...
				if($package->get('item_voter') > 0)
				{
					$package->add('item_star', (int)($package->get('item_voted')/$package->get('item_voter')));
				}
				else
				{
					$package->add('item_star',0);
				}
				...
				{
					Context::set('voted', $voted = $oResourceModel->hasVoted($this->module_srl, $package->get('package_srl'), $package->get('item_srl'), $logged_info->member_srl));
				}
			}
